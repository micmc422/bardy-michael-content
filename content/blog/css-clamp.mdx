...
<Faq faqTitle="FAQ : Typographie fluide avec CSS clamp()" faq={[
  { title: "Quand utiliser clamp() plutÃ´t que des media queries classiques ?", content: "clamp() est particuliÃ¨rement utile quand vous voulez une variation fluide et continue d'une propriÃ©tÃ© (taille de police, espacement, largeur) sans avoir Ã  gÃ©rer plusieurs points de rupture manuellement. C'est idÃ©al pour la typographie principale, les conteneurs majeurs ou les paddings critiques. Pour des Ã©lÃ©ments secondaires ou des cas oÃ¹ une taille fixe suffit, une approche en rem reste parfaitement pertinente." },
  { title: "clamp() est-il bien supportÃ© sur les navigateurs de mes clients en rÃ©gion Occitanie ou ailleurs ?", content: "Oui, clamp() est supportÃ© par tous les navigateurs modernes (Chrome, Firefox, Safari, Edge) depuis mi-2020. Si vous avez besoin de couvrir des versions trÃ¨s anciennes, il est possible d'ajouter une rÃ¨gle de fallback juste avant. Pour un site classique Ã  destination du grand public, vous pouvez l'utiliser en toute confiance dÃ¨s aujourd'hui." },
  { title: "Peut-on combiner clamp() avec calc() pour des calculs plus complexes ?", content: "Absolument. La valeur intermÃ©diaire de clamp() peut Ãªtre une expression calc(), ce qui ouvre des possibilitÃ©s trÃ¨s flexibles. Par exemple : clamp(1rem, calc(1rem + 2vw), 2rem) pour une taille de police qui grandit avec l'Ã©cran tout en restant dans des bornes prÃ©cises. C'est une approche que j'utilise frÃ©quemment dans mes projets Next.js pour garantir une expÃ©rience lisse sur mobile comme sur grand Ã©cran." },
  { title: "clamp() peut-il ralentir l'affichage de mon site ?", content: "Dans la grande majoritÃ© des cas, l'impact est nÃ©gligeable. Par contre, Ã©vitez d'imbriquer trop de clamp() et de calc() complexes dans des parties trÃ¨s sollicitÃ©es, car le navigateur peut mettre un peu plus de temps Ã  recalculer lors d'un resize trÃ¨s rapide. Pour des usages standards (titres, conteneurs, paddings), le gain de lisibilitÃ© et de fluiditÃ© l'emporte largement." }
]} />
