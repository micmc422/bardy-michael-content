...
<Faq title="FAQ : Typographie fluide avec CSS clamp()" faq={[ { title: "Quand utiliser clamp() plutôt que des media queries classiques ?", content: "clamp() est particulièrement utile quand vous voulez une variation fluide et continue d'une propriété (taille de police, espacement, largeur) sans avoir à gérer plusieurs points de rupture manuellement. C'est idéal pour la typographie principale, les conteneurs majeurs ou les paddings critiques. Pour des éléments secondaires ou des cas où une taille fixe suffit, une approche en rem reste parfaitement pertinente." }, { title: "clamp() est-il bien supporté sur les navigateurs de mes clients en région Occitanie ou ailleurs ?", content: "Oui, clamp() est supporté par tous les navigateurs modernes (Chrome, Firefox, Safari, Edge) depuis mi-2020. Si vous avez besoin de couvrir des versions très anciennes, il est possible d'ajouter une règle de fallback juste avant. Pour un site classique à destination du grand public, vous pouvez l'utiliser en toute confiance dès aujourd'hui." }, { title: "Peut-on combiner clamp() avec calc() pour des calculs plus complexes ?", content: "Absolument. La valeur intermédiaire de clamp() peut être une expression calc(), ce qui ouvre des possibilités très flexibles. Par exemple : clamp(1rem, calc(1rem + 2vw), 2rem) pour une taille de police qui grandit avec l'écran tout en restant dans des bornes précises. C'est une approche que j'utilise fréquemment dans mes projets Next.js pour garantir une expérience lisse sur mobile comme sur grand écran." }, { title: "clamp() peut-il ralentir l'affichage de mon site ?", content: "Dans la grande majorité des cas, l'impact est négligeable. Par contre, évitez d'imbriquer trop de clamp() et de calc() complexes dans des parties très sollicitées, car le navigateur peut mettre un peu plus de temps à recalculer lors d'un resize très rapide. Pour des usages standards (titres, conteneurs, paddings), le gain de lisibilité et de fluidité l'emporte largement." }, ]} />
